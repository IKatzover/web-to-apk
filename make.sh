...
apk() {
    ...
    if [ -f "app/build/outputs/apk/release/app-release.apk" ]; then
        ...
        echo -e "Package: ${BLUE}${full_appname}${NC}"
        echo -e "App name: ${BLUE}$(grep -o 'app_name">[^<]*' app/src/main/res/values/strings.xml | cut -d'>' -f2)${NC}"
        echo -e "URL: ${BLUE}$(grep 'String mainURL' app/src/main/java/$(echo $full_appname | tr '.' '/')/*.java | cut -d'"' -f2)${NC}"
        echo -e "${BOLD}----------------${NC}"
    else
        error "Build failed"
    fi
...
test() {
    ...
    try "adb shell am start -n $full_appname/.MainActivity"
    echo "=========================="
...
chid() {
    ...
    # Accept either a full package ID (com.example.myapp) or a short name (myapp)
    if [[ "$new_pkg" =~ \. ]]; then
        # Full package ID provided — validate it
        if ! [[ "$new_pkg" =~ ^[a-zA-Z][a-zA-Z0-9_]*(\.[a-zA-Z][a-zA-Z0-9_]*)+$ ]]; then
            error "Invalid package ID '$new_pkg'. Must be a valid Android package ID like com.example.myapp"
        fi
        local new_full_pkg="$new_pkg"
    else
        # Short name provided — keep old behaviour, wrap in com.<name>.webtoapk
        if ! [[ "$new_pkg" =~ ^[a-zA-Z][a-zA-Z0-9_]*$ ]]; then
            error "Invalid application ID. Use only letters, numbers and underscores, start with a letter"
        fi
        local new_full_pkg="com.$new_pkg.webtoapk"
    fi

    # Derive java source subfolders
    local new_pkg_path="${new_full_pkg//./\/}"
    local old_pkg_path="${full_appname//./\/}"

    if [ "$new_full_pkg" = "$full_appname" ]; then
        return 0
    fi

    info "Old package: $full_appname"
    info "Renaming to: $new_full_pkg"

    # Replace package name in all relevant files
    find . -type f \( -name '*.gradle' -o -name '*.java' -o -name '*.xml' \) -exec \
        sed -i "s|${full_appname}|${new_full_pkg}|g" {} +

    # Move java source directory
    local old_java_dir="app/src/main/java/${old_pkg_path}"
    local new_java_dir="app/src/main/java/${new_pkg_path}"

    if [ -d "$old_java_dir" ] && [ "$old_java_dir" != "$new_java_dir" ]; then
        mkdir -p "$(dirname "$new_java_dir")"
        mv "$old_java_dir" "$new_java_dir"
    fi

    full_appname="$new_full_pkg"
    appname="${new_full_pkg##*.}"
...
full_appname=$(grep -Po '(?<=applicationId ")[^"]*' app/build.gradle)
